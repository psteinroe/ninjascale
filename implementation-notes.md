...
- Retention is computed from configuration as the largest sustained window plus two bucket durations. Expired bucket history is pruned during writes and snapshots; one latest sample per series remains available so target policies keep their prior latest-value semantics. The store rejects zero-time, non-finite, future, and already-expired arrivals rather than manufacturing values.
